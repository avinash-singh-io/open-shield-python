...
from open_shield.api.fastapi import OpenShieldMiddleware, get_user_context, RequireScope
from open_shield.domain.entities import UserContext, User, Token
from open_shield.adapters import OpenShieldConfig
from open_shield.domain.exceptions import TokenValidationError, AuthorizationError
...
def client(mock_token_service):
    ...
    with patch("open_shield.api.fastapi.middleware.TokenService") as MockServiceClass, \
         patch("open_shield.api.fastapi.middleware.OIDCDiscoKeyProvider"), \
         patch("open_shield.api.fastapi.middleware.PyJWTValidator"):
         
        MockServiceClass.return_value = mock_token_service
...
def test_protected_endpoint_invalid_token(client, mock_token_service):
    mock_token_service.validate_and_extract.side_effect = TokenValidationError("Invalid")
