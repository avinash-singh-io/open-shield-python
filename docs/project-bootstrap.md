...
> **Purpose**: Feed this document to an agent at the start of any new repository.
> The agent will scaffold the full spec-driven project structure — folders, rules, workflows,
> tracking system, and CLAUDE.md — so that humans and agents can develop together from day one.
>
...
During implementation, decisions change, scope shifts, and new things are discovered. Without a system, these changes drift — docs reference stale information, future phases plan against outdated assumptions. The solution:

1. **Capture now** — append to phase history during implementation
2. **Sync later** — propagate changes to docs at phase completion via `/sync-docs`

This keeps the agent focused on implementation while ensuring nothing is lost.
...
├── docs/
│   ├── <project-name>.md              # Vision / master plan document
...
- If on `main` or `staging`, **auto-create a feature branch** before making changes:
  - Phase work: `phase-N-shortname` (e.g., `phase-0-bootstrap`)
...
4. Build phase topic index:
   ...
   - Add/update the phase entry in `specs/phases/index.json`:
     ...
     ```
   - Create the phase history file: `specs/phases/phase-N-name/history.md` from the template (see Step 10)
...
7. Create git branch and initial commit:
   ```bash
...
8. Update `specs/changelog/YYYY-MM.md` with phase start entry.
```
...
5. Create retrospective in the phase directory:
...
7. Commit all remaining changes on phase branch and push:
   ```bash
   git add -A
...
8. Merge phase branch → staging (ask user for approval first):
   ```bash
...
9. Merge staging → main (ask user for approval first):
   ```bash
   git checkout main
...
10. Tag on main + create GitHub Release (ask user for approval first):
    ```bash
    git checkout main
...
Commit: `docs(phase-N): sync docs from phase history`
...
3. Check for dependencies:
   ...
   - Items that have been `open` for 3+ months without progress
...
```json
{
  ...
  "topics": {
    "example-topic": {
      "files": [
        {"path": "docs/<project-name>.md", "section": "Section Name"},
...
> **Current Phase**: Phase 0 — <Phase Name> (`not-started`)
> **Latest Release**: None
> **Health**: On Track
...
|-------|------|--------|-----------------|
| 1 | <Phase 1 Name> | Not Started | <deliverables> |
...
|-------|------|--------|-----------|
| 0 | <Phase 0 Name> | Not Started | [phase-0-<name>/](phase-0-<name>/) |
...
### `specs/phases/phase-0-<name>/overview.md`
...
### `specs/phases/phase-0-<name>/plan.md`
...
```
<show the directory structure that will be created>
```
...
### `specs/phases/phase-0-<name>/tasks.md`
...
### `specs/phases/phase-0-<name>/history.md` (created by `/start-phase`)

```markdown
# Phase 0 — <Name>: Implementation History
...
|-------|------|--------|-------|-----|--------|
| 0 | <Name> | Not Started | YYYY-MM-DD | YYYY-MM-DD | X days |
...
> Project management for <Project Name>. The single source of truth for all work.
...
- scripts/check-history-reminder.sh for phase history enforcement
...
1. **Write your vision document** — put it in `docs/<project-name>.md`. This is the master plan the agent reads to understand the full scope of what needs to be built.
...
4. **Iterate** — during each phase:
   ...
   - Use `/log <message>` to manually record anything the agent missed
...
│                                                                  │
│  CLAUDE.md → Rules 1-9 (orient, track, history, git, sync)     │
│  specs/status.md → Current phase, blockers, P0 items            │
│                                                                  │
├─────────────────────────────────────────────────────────────────┤
│                    DURING IMPLEMENTATION                         │
│                                                                  │
│  Agent writes code → auto-updates tasks.md, changelog           │
│  Agent discovers issues → auto-adds to backlog.md               │
│  Agent makes decisions → auto-appends to history.md             │
│  Hook fires on Edit/Write → reminds agent if history needed     │
│                                                                  │
├─────────────────────────────────────────────────────────────────┤
│                    AT PHASE COMPLETION                            │
│                                                                  │
│  /sync-docs reads:                                               │
│    history.md → what changed (topics, types)                     │
│    index.json → which phases are affected                        │
│    impact-map.json → which docs are affected                     │
│  Then: targeted edits to only affected files                     │
│                                                                  │
│  /complete-phase:                                                │
│    verify tasks → retrospective → update tracking → release     │
│    phase-branch → staging → main → tag → GitHub Release         │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
