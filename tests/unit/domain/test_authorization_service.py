...
def test_require_scope_success(user_context: UserContext) -> None:
    service = AuthorizationService()
    service.require_scope(user_context, "read:users")


def test_require_scope_failure(user_context: UserContext) -> None:
    service = AuthorizationService()
    with pytest.raises(AuthorizationError):
        service.require_scope(user_context, "write:users")


def test_require_any_role_success(user_context: UserContext) -> None:
    service = AuthorizationService()
    service.require_any_role(user_context, ["admin", "editor"])


def test_require_any_role_failure(user_context: UserContext) -> None:
    service = AuthorizationService()
    with pytest.raises(AuthorizationError):
